...
def replace_rare_labels(df: pd.DataFrame, cat_vars: list, target: str):
...
def replace_categoricals(df:pd.DataFrame, cat_vars:list):
    def replace_categories(d: pd.DataFrame, var: str, target: str):
        ordered_labels = d.groupby([var])[target].mean().sort_values().index
        ordinal_label = {k: i for i, k in enumerate(ordered_labels, 0)}
        d[var] = d[var].map(ordinal_label)

    for var in cat_vars:
        replace_categories(df, var, 'price')
